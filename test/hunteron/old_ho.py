...
class TbHeadhunterBlack(db.Model):
    ...
    update_user_id = db.Column(db.BigInteger)
    update_time = db.Column(db.DateTime, server_default=db.FetchedValue())
    black_status = db.Column(db.Integer, server_default=db.FetchedValue(), info='失效状态 0:失效, 1:生效')
    reason = db.Column(db.String(500), info='关小黑屋原因')
...
class TbHeadhunterWaring(db.Model):
    __tablename__ = 'tb_headhunter_waring'

    id = db.Column(db.BigInteger, primary_key=True)
    defunct = db.Column(db.Integer, server_default=db.FetchedValue(), info='0未删除，1 删除')
    create_user_id = db.Column(db.BigInteger, info='创建人')
    create_time = db.Column(db.DateTime, server_default=db.FetchedValue(), info='创建时间')
    update_user_id = db.Column(db.BigInteger, info='修改人')
    update_time = db.Column(db.DateTime, server_default=db.FetchedValue(), info='修改时间')
    hh_id = db.Column(db.BigInteger, index=True, info='猎头id')
    reason = db.Column(db.String(500), info='设置原因')
